...
public class LouisvilleAdapter extends ArrayAdapter<LouisvilleClass> {
    int quantity = 0;
    int i = 0;
    int value = 0;
    String hold = "";
    ...
    public LouisvilleAdapter(Activity context, ArrayList<LouisvilleClass> louisvilleItem) {
        super(context, 0, louisvilleItem);
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        LouisvilleClass currentLouisvilleItem = getItem(position);

        ImageView imageView = listItemView.findViewById(R.id.item_image);
        imageView.setImageResource(currentLouisvilleItem.getImageResourceId());

        TextView nameTextView = listItemView.findViewById(R.id.item_name);
        nameTextView.setText(currentLouisvilleItem.getItemName());

        TextView priceTextView = listItemView.findViewById(R.id.item_price);
        priceTextView.setText("Price " + currentLouisvilleItem.getItemPrice());
        ...
        hold = currentLouisvilleItem.getItemquantity();
        ...
        cart_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {        //Database Items not changed from original items
                ...
                if (quantity != 0) { //if quan < 0 or equals to 0
                    if (pos == 0) {
                        boolean isinserted = appDatabaseHelper.addTripToTable("Franfort Trolley Tour", String.valueOf(quantity), String.valueOf(550 * quantity));
                        if (isinserted) {
                    ...
                    if (pos == 1) {
                        boolean isinserted = appDatabaseHelper.addTripToTable("The Muhammad Ali Center Tour", String.valueOf(quantity), String.valueOf(550 * quantity));
                        if (isinserted) {
                            int price = 1;
                    ...
                    if (pos == 2) {
                        boolean isinserted = appDatabaseHelper.addTripToTable("The Old 502 Winery Tour", String.valueOf(quantity), String.valueOf(250 * quantity));
                        if (isinserted) {
                            int price = 1;
                    ...
                    if (pos == 3) {
                        ...
                        boolean isinserted = appDatabaseHelper.addTripToTable("Fairmount Falls Tour", String.valueOf(quantity), String.valueOf(100 * quantity));
                        if (isinserted) {
                            int price = 1;
                    ...
                    if (pos == 4) {//duplicated to match the 5 items
                        ...
                        boolean isinserted = appDatabaseHelper.addTripToTable("The Howard Steamboat Museum Tour", String.valueOf(quantity), String.valueOf(100 * quantity));
                        if (isinserted) {
                            int price = 1;
                            order_details[i] = "Id " + counter + " Tour price " + 100 * quantity + " ";
                            counter++;
